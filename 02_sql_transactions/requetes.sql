/* =============================================================
   Requetes - alimentation, mises a jour et analyses
   ============================================================= */
...
/* ---- 2. Mise a jour : recalcul des soldes -------------------- */
UPDATE comptes
SET solde = (SELECT COALESCE(SUM(montant), 0)
             FROM transactions
             WHERE transactions.compte_id = comptes.compte_id);

/* ---- 3. Suppression d'une transaction annulee --------------- */
DELETE FROM transactions WHERE transaction_id = 7;

/* ---- 4. Analyses -------------------------------------------- */

-- Solde total par client (jointure + agregation)
...
-- Debits et credits par compte
SELECT compte_id,
       SUM(CASE WHEN montant > 0 THEN montant ELSE 0 END) AS credits,
       SUM(CASE WHEN montant < 0 THEN montant ELSE 0 END) AS debits
FROM transactions
GROUP BY compte_id
ORDER BY compte_id;
